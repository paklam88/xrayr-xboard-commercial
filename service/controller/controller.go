...
func (c *Controller) nodeInfoMonitor() (err error) {
	...
	// If nodeInfo changed
	inboundReloaded := false
	if nodeInfoChanged {
		if requiresInboundReload(c.nodeInfo, newNodeInfo) {
			// Remove old tag (listen-port / transport / TLS class changes only)
			oldTag := c.Tag
			...
			// Soft update: keep existing listen socket; apply non-structural fields.
			if c.nodeInfo.SpeedLimit != newNodeInfo.SpeedLimit {
	...
	if inboundReloaded {
		...
		if usersChanged {
			...
			if len(deleted) > 0 {
				...
				// Hot remove via UserManager.RemoveUser (no listen restart)
				err := c.removeUsers(deletedEmail, c.Tag)
				if err != nil {
					c.logger.Print(err)
				}
				if err := c.DeleteLimiterUsers(c.Tag, deletedEmail); err != nil {
					c.logger.Print(err)
				}
				c.unregisterTrafficCounters(deletedEmail)
			}
			if len(added) > 0 {
				// Hot add via UserManager.AddUser (no listen restart)
...
func (c *Controller) userInfoMonitor() (err error) {
	...
	// Memory defense: prune idle limiter caches and unregister stale traffic counters.
	if idleEmails := c.CleanupInactiveLimiter(c.Tag); len(idleEmails) > 0 {
		c.unregisterTrafficCounters(idleEmails)
		c.logger.Printf("GC cleaned %d inactive user caches", len(idleEmails))
	}
